...
- **Visible breadcrumbs:** `src/components/PageBreadcrumb.jsx` on guides, FAQ,
  ...
  internal links (guide, vaccination, mom care, sources). Milestone guides with
  `milestoneMonth` show the same related-link row (tracker, vaccination, mom care, sources).
