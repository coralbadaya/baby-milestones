...
- **`ContentPage.jsx`** — renders structured legal/company pages from
  `src/data/legalContent.js` `PAGES`. Optional `breadcrumb` slot sits **inside**
  `.content-page` so crumbs share the 760px column (never full-bleed under `<main>`).
- **`PageBreadcrumb.jsx`** — quiet trail for guides, FAQ, legal/company. Centered
  on `.content-page`; left-aligned in `PageSection` (Guides hub). Phosphor caret separator.
...
- **Homepage:** `index.html` + Home share the same title/description (The Art of
  ...
  Yarn Trails is the eyebrow. Copy lives in `src/data/homepageCopy.js` and is
  prerendered into `dist/index.html` so crawlers see a real article body
  (what it is, who it is for, what you can track). Welcome-mode Home renders
  the same intro; logged-in Today does not.
...
- **Visible breadcrumbs:** `src/components/PageBreadcrumb.jsx` on guides, FAQ,
  legal/company pages. Always inside the page column (`.content-page` or
  `PageSection`), never a sibling of the centered article. Content pages center
  the trail with the hero; the Guides hub left-aligns it with the section header.
  Month pages keep the timeline back control and add related internal links
  (guide, vaccination, mom care, sources). Milestone guides with `milestoneMonth`
  show the same related-link row (tracker, vaccination, mom care, sources).
